...
class VVHeaderVW extends React.Component {
  constructor() {
	...
	this.wallets = [{
		...
		rpc: {
			[config.networkId]: config.rpc_url
		}
	},{ 
	...
	this.onboard = Onboard({
		dappId: config.onboard,       // [String] The API key created by step one above
		networkId: config.networkId,  // [Integer] The Ethereum network ID your Dapp uses.
		darkMode: true,
		walletSelect: {
  ...
  depositAuction = async (walletInfo) => {
  	let that = this;
  	try {
  		const currentContract = new ethers.Contract(config.platform_contract_address, VVPlatform, this.signer)
  ...
  lockAuction = async (walletInfo) => {
  	...
  	try {
  		...
  		const currentContract = new ethers.Contract(config.platform_contract_address, VVPlatformBid, this.signer)
		getOptions("gas_fee").then(optionObj=>{
			let gas_fee = parseInt(optionObj.result.value);
  ...
  deleteAuction = (auctionInfo) => {
	// const currentContract = new ethers.Contract(config.auction_address, VVAuction, this.signer)
  ...
  approveNFT = (itemInfo) => {
	let that = this;
	this.approveContract(config.contract_address,function(approveResult){
  ...
  sendMoneyToOwner = (addressArray, priceArray, priceValue, callback) => {
    const currentContract = new ethers.Contract(config.multi_contract_address, VVSend, this.signer)
	getOptions("gas_fee").then(optionObj=>{
		let gas_fee = parseInt(optionObj.result.value);
  ...
  approveContract = (contract_address, callback) => {
	...
	console.log(config.main_address);
    currentContract.isApprovedForAll(this.address,config.main_address).then((isApproved) => {
      if(isApproved) {
        var resulter = {
          status: true,
          result: isApproved,
		...
		getOptions("gas_fee").then(optionObj=>{
			let gas_fee = parseInt(optionObj.result.value);
			currentContract.setApprovalForAll(config.main_address, true,{
				gasLimit: gas_fee * 100000, 
				gasPrice: ethers.utils.parseUnits(gas_fee.toString(), 'gwei'), 
			}).then((result) => {
				var resulter = {
			  ...
			  { (this.state.hasResult && !this.state.searchloading) &&
				...
				<div className="space-x-20 d-flex flex-column flex-md-row sm:space-y-20">
					...
					{ !this.state.isLoggedIn &&
							<a className="btn btn-white" onClick={this.LoginAction} href="javascript:void(0)">
					...
					{ this.state.isLoggedIn &&
						...
						this.showDropDownMenu()
										...
										<span>{this.state.user_balance} MATIC</span></div>
...
export default connect(mapStateToProps, {actionNotifyUser})(withRouter(VVHeaderVW))
